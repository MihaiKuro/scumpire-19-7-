...
function calculeazaPretFinal(scumpireType) {
    ...
    rezultatDiv.innerHTML = "Pretul final este: " + pretFinal.toFixed(4);

    // Adaugă în istoric cu inversarea ordinii
    let listItem = document.createElement("li");
    listItem.appendChild(document.createTextNode("Calcul " + (scumpireType === '19' ? arr19.length +1 : arr7.length +1) + ": " + pretFinal.toFixed(4)));

    istoricList.insertBefore(listItem, istoricList.firstChild);

    // Adaugă la istoric
    if (scumpireType === '19') {
        arr19.unshift(pretFinal.toFixed(4));
    } else if (scumpireType === '7') {
        arr7.unshift(pretFinal.toFixed(4));
    }
